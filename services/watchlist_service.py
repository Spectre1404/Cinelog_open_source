...
def add_to_watchlist(user_id, film_id, public=False):
    ...
    Args:
        ...
        public (bool, optional): Visibility of the entry. Defaults to False
            (private) so callers opt in to sharing; see the visibility decision
            in the PR review responses.
    ...
    # Deduplicate: a film should appear on a user's watchlist at most once.
...
def get_watchlist(user_id):
    ...
    Sort order decision (per PR review): watchlists default to "date added"
    descending rather than alphabetical, since users most want to see what
    they recently saved. This also matches get_collection's ordering.
